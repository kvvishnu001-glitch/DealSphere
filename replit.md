...
- **Shared Layout Components**: Created reusable Header (`client/src/components/layout/header.tsx`) with mobile hamburger menu and Footer (`client/src/components/layout/footer.tsx`) with responsive 4-column grid. All 10 public pages now use these shared components instead of inline duplicated markup.
- **Mobile Responsive Improvements**: Filter bar uses smaller touch-friendly controls on mobile, deal cards have responsive image heights/padding/fonts, descriptions hidden on small screens, hero section scales properly, sticky header height adjusted for mobile (h-14 vs h-16).
- **URL Health Checker**: Automated system checks all deal URLs every 2 hours for accessibility. After 2 consecutive failures, deals are flagged as pending review. Deals with broken URLs for 24+ hours are automatically removed. SSRF-safe with IP/hostname validation. Admin API endpoints at /api/admin/url-health for stats, /api/admin/url-health/check to trigger manual check, /api/admin/url-health/cleanup for stale deal removal.
